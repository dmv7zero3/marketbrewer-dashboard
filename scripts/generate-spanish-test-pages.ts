#!/usr/bin/env ts-node
/**
 * Generate Spanish Test Pages for Street Lawyer Magic
 *
 * Generates 4 Spanish versions of the English test pages for quality review.
 *
 * Usage: npx ts-node scripts/generate-spanish-test-pages.ts
 */
...
// Spanish versions of the 4 English test pages
const TEST_PAGES = [
  { keyword: "abogado de DUI", city: "Bethesda", state: "MD", language: "es" },
  { keyword: "abogado de delitos de drogas", city: "Silver Spring", state: "MD", language: "es" },
  { keyword: "abogado de violencia doméstica", city: "Rockville", state: "MD", language: "es" },
  { keyword: "abogado defensor criminal", city: "Washington", state: "DC", language: "es" },
];
...
async function main() {
  console.log("\n=== Street Lawyer Magic - Spanish Test Page Generation ===\n");
  ...
  try {
    ...
    // Generate test pages
    console.log(`Generating ${TEST_PAGES.length} Spanish test pages...\n`);
    ...
    for (let i = 0; i < TEST_PAGES.length; i++) {
      ...
      try {
        ...
        // Create filename with Spanish prefix
        const filename = `es-${i + 1}-${toSlug(testPage.keyword)}-${toCityStateSlug(testPage.city, testPage.state)}.md`;
...
`;
    ...
    console.log("\nReview these Spanish pages before running full generation.");
