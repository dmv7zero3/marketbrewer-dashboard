 ...
 */
router.post('/:jobId/claim', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (job.status === 'completed' || job.status === 'cancelled') {
      throw new HttpError(409, 'Job is already completed or cancelled', 'JOB_ALREADY_CLAIMED');
    }
    ...
    // Atomic claim: Find and update a queued page in one operation
    // Using a transaction for atomicity
    const claimedPage = db.transaction(() => {
      // Find the next queued page
      const page = dbGet<JobPage>(
        `SELECT * FROM job_pages 
         WHERE job_id = ? AND status = 'queued' AND attempts < ?
      ...
      dbRun(
        `UPDATE job_pages 
         SET status = 'processing', worker_id = ?, claimed_at = ?, attempts = attempts + 1
         WHERE id = ?`,
        [data.worker_id, now, page.id]
      );

      // Return the updated page
      return dbGet<JobPage>('SELECT * FROM job_pages WHERE id = ?', [page.id]);
    })();
 ...
 */
router.put('/:jobId/pages/:pageId/complete', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (page.status !== 'processing') {
      throw new HttpError(409, 'Page is not in processing state', 'JOB_ALREADY_CLAIMED');
    }
    ...
    if (job && job.completed_pages + job.failed_pages >= job.total_pages) {
      // Job is complete
      const finalStatus = job.failed_pages === job.total_pages ? 'failed' : 'completed';
      dbRun(
    ...
    const updatedPage = dbGet<JobPage>('SELECT * FROM job_pages WHERE id = ?', [pageId]);
 ...
 */
router.get('/:jobId/pages', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { jobId } = req.params;
 ...
 */
router.post('/:jobId/release-stale', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { jobId } = req.params;
